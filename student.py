...
class Student(object):
    ...
    def __init__(self, semesters_left=None, major_name = 'MechE' , courses_taken = ['ModSim', 'ISIM', 'Design Nature', 'AHSE Foundation'], courses_left=[]):
        self.Major = major_objects[major_name]
        self.courses_taken = [course_objects[course_name] for course_name in courses_taken]
    ...
    def major_requirements_left(self):
        if self.courses_left==None:
            ...
            pass
        ###TODO: resolve: What are we looking for when they pass in "courses_left"? Non-required courses that they want to take? Required and non-required? Nothing at all?""
        for course in self.courses_left:
            if course in self.courses_taken and self.Major.major_requirements:
                self.courses_left -= self.courses_taken
	...
	semester_list = []
	for number in range(self.semesters_left):
	...
	for course in course_list:
		for semester in semester_list:
			if course.required_semester == semester.number: #If we already know when the class must be taken,
	...
	for i in range(1000):
		...
		for semester in semester_list:
			...
			#Assign random semester numbers to what's left in course_list (1 or 2 in this case)
			sem_indices = numpy.where(random_semester_assigns == semester.number)[0]
	...
	#Now reassign with the chosen best courses for each semester
	for semester in semester_list:
		semester.difficulty = 0
		semester.courses[:] = []
		sem_indices = numpy.where(best_distribution_of_classes == semester.number)[0]
		semester.courses = semester.base_courses + [course_list[i] for i in sem_indices]
